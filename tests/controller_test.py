...
class TestImport:
    def test_import(self, coll_vdirs):
        ...
        assert utils.BERLIN.localize(dt.datetime(2014, 7, 14, 7, 0)) in [ev.start for ev in events]
    ...
    def test_mix_datetime_types(self, coll_vdirs):
        ...
        assert len(events) == 2
        events = sorted(events)
        assert events[0].start_local == utils.BERLIN.localize(dt.datetime(2015, 5, 30, 12, 0))
        assert events[0].end_local == utils.BERLIN.localize(dt.datetime(2015, 5, 30, 16, 0))
        assert events[1].start_local == utils.BERLIN.localize(dt.datetime(2015, 6, 2, 12, 0))
        assert events[1].end_local == utils.BERLIN.localize(dt.datetime(2015, 6, 2, 16, 0))
