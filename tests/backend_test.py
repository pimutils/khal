...
def test_event_rrule_recurrence_id():
    ...
    assert list(events) == []
    dbi.update(
        _get_text("event_rrule_recuid"), href="12345.ics", etag="abcd", calendar=calname
    )
...
def test_event_recuid_no_master():
    ...
    dbi = backend.SQLiteDb([calname], ":memory:", locale=LOCALE_BERLIN)
    dbi.update(
        _get_text("event_dt_recuid_no_master"),
        href="12345.ics",
        etag="abcd",
        calendar=calname,
    )
...
def test_this_and_prior():
    ...
    with pytest.raises(UpdateFailed):
        dbi.update(
            event_rrule_this_and_prior, href="12345.ics", etag="abcd", calendar=calname
        )
...
def test_event_rrule_this_and_future():
    dbi = backend.SQLiteDb([calname], ":memory:", locale=LOCALE_BERLIN)
    dbi.update(
        event_rrule_this_and_future, href="12345.ics", etag="abcd", calendar=calname
    )
    assert dbi.list(calname) == [("12345.ics", "abcd")]
    events = dbi.get_localized(
        BERLIN.localize(dt.datetime(2014, 4, 30, 0, 0)),
...
def test_event_rrule_multi_this_and_future_allday():
    ...
    events = sorted(
        dbi.get_floating(
            dt.datetime(2014, 4, 30, 0, 0), dt.datetime(2014, 9, 27, 0, 0)
        ),
    )
    assert len(events) == 6
...
recuid_this_future = icalendar.Event.from_ical(
    """BEGIN:VEVENT
UID:event_rrule_recurrence_id
SUMMARY:Arbeit
RECURRENCE-ID;RANGE=THISANDFUTURE:20140707T050000Z
...
recuid_this_future_duration = icalendar.Event.from_ical(
    """BEGIN:VEVENT
UID:event_rrule_recurrence_id
SUMMARY:Arbeit
RECURRENCE-ID;RANGE=THISANDFUTURE:20140707T050000Z
...
def test_update_one_should_not_affect_others():
    ...
    events = db.get_floating(
        dt.datetime(2015, 4, 9, 0, 0), dt.datetime(2015, 4, 10, 0, 0)
    )
    assert len(list(events)) == 1
    db.update(event_rrule_multi_this_and_future_allday, href="second", calendar=calname)
    events = list(
        db.get_floating(dt.datetime(2015, 4, 9, 0, 0), dt.datetime(2015, 4, 10, 0, 0))
    )
    assert len(events) == 1
...
def test_check_support():
    ...
    if pkg_resources.get_distribution(
        "icalendar"
    ).parsed_version <= pkg_resources.parse_version("3.9.1"):
...
def test_birthdays():
    ...
    events = list(
        db.get_floating(
            dt.datetime(2016, 3, 11, 0, 0), dt.datetime(2016, 3, 11, 23, 59, 59, 999)
        )
    )
    assert "SUMMARY:Unix's birthday" in events[0][0]
