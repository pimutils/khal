...
class TestVdirsyncerCompat:
    def test_list(self, coll_vdirs):
        ...
        event = Event.fromString(
            _get_text("event_d"), calendar=cal1, locale=LOCALE_BERLIN
        )
...
class TestCollection:
    ...
    def test_empty(self, coll_vdirs):
        ...
        assert (
            list(
                coll.get_localized(
                    utils.BERLIN.localize(start), utils.BERLIN.localize(end)
                )
            )
            == []
        )
    ...
    def test_search_recurrence_id_only_multi(self, coll_vdirs):
        ...
        assert len(events) == 2
        assert (
            events[0].format("{start} {end} {title}", dt.date.today())
            == "30.06. 07:30 30.06. 12:00 Arbeit\x1b[0m"
        )
        assert (
            events[1].format("{start} {end} {title}", dt.date.today())
...
def test_birthdays(coll_vdirs_birthday, sleep_time):
    ...
    assert (
        list(
            coll.get_floating(
                dt.datetime(1971, 3, 11), dt.datetime(1971, 3, 11, 23, 59, 59)
            )
        )
        == []
    )
    ...
    assert (
        ...
        == list(coll.get_floating(dt.datetime(2012, 3, 11), dt.datetime(2012, 3, 11)))[
            0
        ].summary
    )
    assert (
        "Unix's 42nd birthday"
        == list(coll.get_floating(dt.datetime(2013, 3, 11), dt.datetime(2013, 3, 11)))[
            0
        ].summary
    )
    assert (
        "Unix's 43rd birthday"
        == list(coll.get_floating(dt.datetime(2014, 3, 11), dt.datetime(2014, 3, 11)))[
            0
        ].summary
    )
...
def test_birthdays_29feb(coll_vdirs_birthday, sleep_time):
    ...
    assert (
        list(coll.get_floating(dt.datetime(2001, 1, 1), dt.datetime(2001, 12, 31)))[
            0
        ].summary
        == "leapyear's 1st birthday (29th of Feb.)"
    )
    assert (
        list(coll.get_floating(dt.datetime(2002, 1, 1), dt.datetime(2002, 12, 31)))[
            0
        ].summary
        == "leapyear's 2nd birthday (29th of Feb.)"
    )
    assert (
        list(coll.get_floating(dt.datetime(2003, 1, 1), dt.datetime(2003, 12, 31)))[
            0
        ].summary
        == "leapyear's 3rd birthday (29th of Feb.)"
    )
    assert (
        list(coll.get_floating(dt.datetime(2023, 1, 1), dt.datetime(2023, 12, 31)))[
            0
        ].summary
