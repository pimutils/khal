...
event_today = event_allday_template.format(today.strftime("%Y%m%d"), tomorrow.strftime("%Y%m%d"))
...
class TestCalendar:
    ...
    def test_db_needs_update(self, coll_vdirs, sleep_time):
        ...
        for calendar in coll._calendars:
            print(
                f"{calendar}: saved ctag: {coll._local_ctag(calendar)}, "
...
class TestCollection:
    ...
    def test_empty(self, coll_vdirs):
        ...
        assert (
            list(coll.get_localized(utils.BERLIN.localize(start), utils.BERLIN.localize(end))) == []
        )
    ...
    def test_insert_d_no_value(self, coll_vdirs):
        ...
        coll, vdirs = coll_vdirs
        coll.insert(
            Event.fromString(_get_text("event_d_no_value"), calendar=cal1, locale=LOCALE_BERLIN),
            cal1,
        )
        events = list(coll.get_events_on(aday))
        assert len(events) == 1
    ...
    def test_get(self, coll_vdirs):
        ...
        with freeze_time("2016-1-1"):
            assert normalize_component(event_from_db.raw) == normalize_component(
                _get_text("event_dt_simple_inkl_vtimezone")
            )
    ...
    def test_search_recurrence_id_only_multi(self, coll_vdirs):
        ...
        assert len(events) == 2
        assert (
            human_formatter("{start} {end} {title}")(events[0].attributes(dt.date.today()))
            == "30.06. 07:30 30.06. 12:00 Arbeit\x1b[0m"
        )
        assert (
            human_formatter("{start} {end} {title}")(events[1].attributes(dt.date.today()))
            == "07.07. 08:30 07.07. 12:00 Arbeit\x1b[0m"
        )
    ...
    def test_delete_two_events(self, coll_vdirs, sleep_time):
        ...
        coll, vdirs = coll_vdirs
        event1 = Event.fromString(_get_text("event_dt_simple"), calendar=cal1, locale=LOCALE_BERLIN)
        event2 = Event.fromString(_get_text("event_dt_simple"), calendar=cal2, locale=LOCALE_BERLIN)
...
def test_event_different_timezones(coll_vdirs, sleep_time):
    ...
    # next day
    events = coll.get_localized(
        SYDNEY.localize(dt.datetime(2014, 4, 10, 0, 0)),
...
def test_birthdays(coll_vdirs_birthday, sleep_time):
    ...
    assert (
        list(coll.get_floating(dt.datetime(1971, 3, 11), dt.datetime(1971, 3, 11, 23, 59, 59)))
        == []
    )
    sleep(sleep_time)  # Make sure we get a new ctag on upload
    ...
    coll.update_db()
    assert (
        "Unix's 41st birthday"
        == list(coll.get_floating(dt.datetime(2012, 3, 11), dt.datetime(2012, 3, 11)))[0].summary
    )
    assert (
        "Unix's 42nd birthday"
        == list(coll.get_floating(dt.datetime(2013, 3, 11), dt.datetime(2013, 3, 11)))[0].summary
    )
    assert (
        "Unix's 43rd birthday"
        == list(coll.get_floating(dt.datetime(2014, 3, 11), dt.datetime(2014, 3, 11)))[0].summary
    )
...
def test_birthdays_29feb(coll_vdirs_birthday, sleep_time):
    ...
    coll.update_db()
    events = list(coll.get_floating(dt.datetime(2004, 1, 1, 0, 0), dt.datetime(2004, 12, 31)))
    assert len(events) == 1
    assert events[0].summary == "leapyear's 4th birthday (29th of Feb.)"
    assert events[0].start == dt.date(2004, 2, 29)
    events = list(coll.get_floating(dt.datetime(2005, 1, 1, 0, 0), dt.datetime(2005, 12, 31)))
    assert len(events) == 1
    assert events[0].summary == "leapyear's 5th birthday (29th of Feb.)"
    assert events[0].start == dt.date(2005, 3, 1)
    assert (
        list(coll.get_floating(dt.datetime(2001, 1, 1), dt.datetime(2001, 12, 31)))[0].summary
        == "leapyear's 1st birthday (29th of Feb.)"
    )
    assert (
        list(coll.get_floating(dt.datetime(2002, 1, 1), dt.datetime(2002, 12, 31)))[0].summary
        == "leapyear's 2nd birthday (29th of Feb.)"
    )
    assert (
        list(coll.get_floating(dt.datetime(2003, 1, 1), dt.datetime(2003, 12, 31)))[0].summary
        == "leapyear's 3rd birthday (29th of Feb.)"
    )
    assert (
        list(coll.get_floating(dt.datetime(2023, 1, 1), dt.datetime(2023, 12, 31)))[0].summary
        == "leapyear's 23rd birthday (29th of Feb.)"
    )
    assert events[0].start == dt.date(2005, 3, 1)
