...
def test_event_rrule_recurrence_id_invalid_tzid():
    dbi = backend.SQLiteDb([calname], ":memory:", locale=LOCALE_BERLIN)
    dbi.update(
        _get_text("event_rrule_recuid_invalid_tzid"),
        href="12345.ics",
        etag="abcd",
        calendar=calname,
    )
...
def test_event_rrule_recurrence_id_update_with_exclude():
    ...
    dbi.update(_get_text("event_rrule_recuid"), href="12345.ics", etag="abcd", calendar=calname)
    dbi.update(
        _get_text("event_rrule_recuid_update"), href="12345.ics", etag="abcd", calendar=calname
    )
    events = dbi.get_localized(
        BERLIN.localize(dt.datetime(2014, 4, 30, 0, 0)),
        BERLIN.localize(dt.datetime(2014, 9, 26, 0, 0)),
...
def test_event_recuid_no_master():
    ...
    dbi = backend.SQLiteDb([calname], ":memory:", locale=LOCALE_BERLIN)
    dbi.update(
        _get_text("event_dt_recuid_no_master"), href="12345.ics", etag="abcd", calendar=calname
    )
...
def test_no_valid_timezone():
    dbi = backend.SQLiteDb([calname], ":memory:", locale=LOCALE_BERLIN)
    dbi.update(
        _get_text("event_dt_local_missing_tz"), href="12345.ics", etag="abcd", calendar=calname
    )
    events = dbi.get_localized(
...
event_rrule_this_and_future = event_rrule_this_and_future_temp.format(
    "20140707T090000", "20140707T180000"
)
...
event_rrule_this_and_future_multi_day_shift = event_rrule_this_and_future_temp.format(
    "20140708T090000", "20140709T150000"
)
...
def test_event_rrule_this_and_future_multi_day_shift():
    dbi = backend.SQLiteDb([calname], ":memory:", locale=LOCALE_BERLIN)
    dbi.update(
        event_rrule_this_and_future_multi_day_shift, href="12345.ics", etag="abcd", calendar=calname
    )
    assert dbi.list(calname) == [("12345.ics", "abcd")]
    events = dbi.get_localized(
        BERLIN.localize(dt.datetime(2014, 4, 30, 0, 0)),
...
event_rrule_this_and_future_allday = event_rrule_this_and_future_allday_temp.format(
    20140708, 20140709
)
...
def test_event_rrule_this_and_future_allday_prior():
    event_rrule_this_and_future_allday_prior = event_rrule_this_and_future_allday_temp.format(
        20140705, 20140706
    )
    dbi = backend.SQLiteDb([calname], ":memory:", locale=LOCALE_BERLIN)
    dbi.update(
...
def test_calc_shift_deltas():
    assert (dt.timedelta(hours=2), dt.timedelta(hours=5)) == backend.calc_shift_deltas(
        recuid_this_future
    )
    assert (dt.timedelta(hours=2), dt.timedelta(hours=4, minutes=30)) == backend.calc_shift_deltas(
...
def test_birthdays():
    ...
    events = list(
        db.get_floating(dt.datetime(2016, 3, 11, 0, 0), dt.datetime(2016, 3, 11, 23, 59, 59, 999))
    )
    assert "SUMMARY:Unix's birthday" in events[0][0]
...
def test_birthdays_no_fn():
    ...
    assert (
        list(
            db.get_floating(
                dt.datetime(1941, 9, 9, 0, 0), dt.datetime(1941, 9, 9, 23, 59, 59, 9999)
            )
        )
        == []
    )
    ...
    events = list(
        db.get_floating(dt.datetime(1941, 9, 9, 0, 0), dt.datetime(1941, 9, 9, 23, 59, 59, 9999))
    )
    assert len(events) == 1
