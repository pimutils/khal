...
class TestTimeDelta2Str:
    ...
    def test_multi(self):
        assert (
            timedelta2str(dt.timedelta(days=6, hours=-3, minutes=10, seconds=-3))
            == "5d 21h 9m 57s"
        )
...
class TestGuessRangefstr:
    ...
    def test_today(self):
        ...
        assert (
            dt.datetime(2016, 9, 19),
            dt.datetime(2016, 9, 21),
            True,
    ...
    def test_start_and_end_date_time(self):
        assert (
            dt.datetime(2016, 1, 1, 10),
            dt.datetime(2017, 1, 1, 22),
            False,
        ) == guessrangefstr("1.1.2016 10:00 1.1.2017 22:00", locale=LOCALE_BERLIN)

    def test_start_and_eod(self):
        start, end = dt.datetime(2016, 1, 1, 10), dt.datetime(
            2016, 1, 1, 23, 59, 59, 999999
        )
        assert (start, end, False) == guessrangefstr(
            "1.1.2016 10:00 eod", locale=LOCALE_BERLIN
        )
