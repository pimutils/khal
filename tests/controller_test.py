...
class TestGetAgenda:
    def test_new_event(self, coll_vdirs):
        ...
        assert ["                 a meeting :: short description\x1b[0m"] == khal_list(
            coll, [], conf, agenda_format=event_format, day_format=""
        )
    ...
    def test_agenda_fail(self, coll_vdirs):
        with freeze_time("2016-04-10 12:33"):
            ...
            with pytest.raises(exceptions.FatalError):
                khal_list(
                    coll, conf=conf, agenda_format=event_format, datepoint=["xyz"]
                )
            with pytest.raises(exceptions.FatalError):
                khal_list(
                    coll, conf=conf, agenda_format=event_format, datepoint=["today"]
                )
    ...
    def test_empty_recurrence(self, coll_vdirs):
        ...
        assert (
            ...
            in "\n".join(
                khal_list(
                    coll, [], conf, agenda_format=event_format, day_format="{name}"
                )
            ).lower()
        )
...
class TestImport:
    ...
    def test_mix_datetime_types(self, coll_vdirs):
        ...
        assert len(events) == 2
        events = sorted(events)
        assert events[0].start_local == utils.BERLIN.localize(
            dt.datetime(2015, 5, 30, 12, 0)
        )
        assert events[0].end_local == utils.BERLIN.localize(
            dt.datetime(2015, 5, 30, 16, 0)
        )
        assert events[1].start_local == utils.BERLIN.localize(
            dt.datetime(2015, 6, 2, 12, 0)
        )
        assert events[1].end_local == utils.BERLIN.localize(
            dt.datetime(2015, 6, 2, 16, 0)
        )
...
def test_start_end():
    with freeze_time("2016-04-10"):
        ...
        assert (start, end) == start_end_from_daterange(
            ("today",), locale=utils.LOCALE_BERLIN
        )
